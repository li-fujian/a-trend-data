...
run_update() {
  ...
  echo "Mode: ${MODE}, incremental bars: ${INCREMENTAL_BARS}, max failed: ${MAX_FAILED_TO_PUBLISH}, min fresh: ${MIN_FRESH_TO_PUBLISH}"

  cd "${JAVA_DIR}"
  "${MVN_BIN}" -q -DskipTests compile
  "${MVN_BIN}" -q exec:java -Dexec.mainClass=DataUpdateCli \
    "-Dexec.args=--repo-root ${REPO_ROOT} --trading-days-only --mode ${MODE} --incremental-bars ${INCREMENTAL_BARS} --max-failed-to-publish ${MAX_FAILED_TO_PUBLISH} --min-fresh-to-publish ${MIN_FRESH_TO_PUBLISH} --min-sleep-ms ${MIN_SLEEP_MS} --max-sleep-ms ${MAX_SLEEP_MS}"
